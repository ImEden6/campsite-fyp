...
export const CustomerInsights: React.FC<CustomerInsightsProps> = ({ data, loading }) => {
  ...
  return (
    <div className="bg-white dark:bg-night-surface rounded-lg shadow p-6">
      <h3 className="text-lg font-semibold text-gray-900 dark:text-primary-100 mb-6">Customer Insights</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="border border-gray-200 dark:border-secondary-700 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray-600 dark:text-secondary-400">Total Customers</span>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-primary-100">{data.totalCustomers.toLocaleString()}</p>
        </div>

        <div className="border border-gray-200 dark:border-secondary-700 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp className="w-5 h-5 text-green-600 dark:text-green-400" />
            <span className="text-sm text-gray-600 dark:text-secondary-400">New Customers</span>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-primary-100">{data.newCustomers.toLocaleString()}</p>
        </div>

        <div className="border border-gray-200 dark:border-secondary-700 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <Repeat className="w-5 h-5 text-purple-600 dark:text-purple-400" />
            <span className="text-sm text-gray-600 dark:text-secondary-400">Retention Rate</span>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-primary-100">{data.retentionRate.toFixed(1)}%</p>
        </div>

        <div className="border border-gray-200 dark:border-secondary-700 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <DollarSign className="w-5 h-5 text-orange-600 dark:text-orange-400" />
            <span className="text-sm text-gray-600 dark:text-secondary-400">Avg Lifetime Value</span>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-primary-100">{formatCurrency(data.averageLifetimeValue)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        {/* Age Groups */}
        <div>
          <h4 className="text-sm font-semibold text-gray-900 dark:text-primary-100 mb-4">Age Distribution</h4>
        ...
        {/* Guest Origins */}
        <div>
          <h4 className="text-sm font-semibold text-gray-900 dark:text-primary-100 mb-4">Guest Origins</h4>
          <div className="relative h-[250px]">
      ...
      <div className="border-t border-gray-200 dark:border-secondary-700 pt-6">
        <h4 className="text-sm font-semibold text-gray-900 dark:text-primary-100 mb-4">Booking Patterns</h4>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Preferred Site Types */}
          <div>
            <h5 className="text-xs font-medium text-gray-600 dark:text-secondary-400 mb-3">Preferred Site Types</h5>
            <div className="space-y-3">
              {data.bookingPatterns.preferredSiteTypes.map((item, index) => (
                <div key={item.type}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm text-gray-900 dark:text-primary-100">{item.type}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-primary-100">
                      {item.count} ({item.percentage.toFixed(1)}%)
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-night-surface-alt rounded-full h-2">
                    <div
            ...
            <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <p className="text-sm text-gray-900 dark:text-primary-100">
          ...
          {/* Seasonal Trends */}
          <div>
            <h5 className="text-xs font-medium text-gray-600 dark:text-secondary-400 mb-3">Seasonal Trends</h5>
