...
import { CATEGORY_OPTIONS, STATUS_BADGE_COLORS } from '../utils/equipmentConstants';
...
export const InventoryManager: React.FC<InventoryManagerProps> = ({
  ...
  const getLowStockThreshold = (equipment: Equipment) => {
    return equipment.availableQuantity <= Math.ceil(equipment.quantity * 0.2);
  };
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="shrink-0 h-10 w-10">
                          {equipment.images?.[0] ? (
                            <img className="h-10 w-10 rounded object-cover" src={equipment.images[0]} alt={equipment.name} />
                          ) : (
                        ...
                        <div className="ml-4">
                          ...
                          {equipment.description && (
                            <div className="text-sm text-gray-500 truncate max-w-xs">{equipment.description}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">{equipment.category.replace(/_/g, ' ')}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge variant={STATUS_BADGE_COLORS[equipment.status as EquipmentStatus] ?? 'default'}>
                        {equipment.status}
                      </Badge>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        {getLowStockThreshold(equipment) && (
          ...
          {data.pagination.pages > 1 && (
            <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200">
              ...
              <div className="flex sm:hidden justify-between flex-1">
                <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>
                  Previous
                </Button>
                <Button
