/**
 * Analytics Service
 * Provides analytics data with mock/demo mode support
 * Toggle via USE_MOCK_DATA environment variable
 */
...
class AnalyticsService {
    ...
    async getDashboardMetrics(dateRange?: DateRange): Promise<DashboardMetrics> {
        if (this.useMockData()) {
            return this.generateMockDashboardMetrics(dateRange);
        }
        // TODO: Implement real data queries with Prisma
        return this.generateMockDashboardMetrics(dateRange);
    }
    ...
    async getRevenueMetrics(dateRange?: DateRange): Promise<RevenueMetrics> {
        if (this.useMockData()) {
            return this.generateMockRevenueMetrics(dateRange);
        }
        // TODO: Implement real data queries with Prisma
        return this.generateMockRevenueMetrics(dateRange);
    }
     ...
     */
    async getOccupancyMetrics(dateRange?: DateRange): Promise<OccupancyMetrics> {
        if (this.useMockData()) {
            return this.generateMockOccupancyMetrics(dateRange);
        }
        // TODO: Implement real data queries with Prisma
        return this.generateMockOccupancyMetrics(dateRange);
    }
    ...
    async getCustomerInsights(dateRange?: DateRange): Promise<CustomerInsights> {
        if (this.useMockData()) {
            return this.generateMockCustomerInsights(dateRange);
        }
        // TODO: Implement real data queries with Prisma
        return this.generateMockCustomerInsights(dateRange);
    }
     ...
     */
    async getSitePerformance(dateRange?: DateRange): Promise<SitePerformance[]> {
        if (this.useMockData()) {
            return this.generateMockSitePerformance(dateRange);
        }
        // TODO: Implement real data queries with Prisma
        return this.generateMockSitePerformance(dateRange);
    }
    ...
    private generateMockDashboardMetrics(dateRange?: DateRange): DashboardMetrics {
        const defaults = getDefaultDateRange();
        const start = dateRange?.startDate || defaults.start;
        const end = dateRange?.endDate || defaults.end;
        ...
        const revenueChange = (seededRandom(seed + 1) * 30) - 10;
    ...
    private generateMockRevenueMetrics(dateRange?: DateRange): RevenueMetrics {
        const defaults = getDefaultDateRange();
        const start = dateRange?.startDate || defaults.start;
        const end = dateRange?.endDate || defaults.end;
        const seed = getDateSeed(start + end);
    ...
    private generateMockOccupancyMetrics(dateRange?: DateRange): OccupancyMetrics {
        const defaults = getDefaultDateRange();
        const start = dateRange?.startDate || defaults.start;
        const end = dateRange?.endDate || defaults.end;
        const seed = getDateSeed(start + end);
    ...
    private generateMockCustomerInsights(dateRange?: DateRange): CustomerInsights {
        const defaults = getDefaultDateRange();
        const start = dateRange?.startDate || defaults.start;
        const end = dateRange?.endDate || defaults.end;
        const days = getDaysBetween(start, end);
    ...
    private generateMockSitePerformance(dateRange?: DateRange): SitePerformance[] {
        const defaults = getDefaultDateRange();
        const start = dateRange?.startDate || defaults.start;
        const end = dateRange?.endDate || defaults.end;
        const days = getDaysBetween(start, end);
        const seed = getDateSeed(start + end);
        const dayMultiplier = Math.sqrt(days / 30);
        ...
        return sites.map((site, index) => {
            ...
            const variation = 0.7 + seededRandom(siteSeed) * 0.6;
